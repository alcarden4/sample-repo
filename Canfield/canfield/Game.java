...
class Game {
    ...
    /** List of previous game states. */
    protected static ArrayList<Game> gstates = new ArrayList<>(1);
    ...
    /** Sets foundation for testing using CARDS. */
    void setFoundation(Card... cards) {
        for (Pile p : _foundation) {
            p.clear();
        }
        foundation(1).move(new Pile(cards), 1);
        _base = foundation(1).top();
    }

    /** Sets reserve for testing with certain CARD. */
    void setReserve(Card card) {
    ...
    /** Method that will undo the move just made. */
    void undo() {

        if (!gstates.isEmpty()) {
            copyFrom((gstates.get(gstates.size() - 1)));
            gstates.remove(gstates.size() - 1);
        } else {
//            throw err("You are at the beginning!");
        }
    }
    ...
    /**
     * Move the top card of the reserve to tableau pile #K, 1 <= K <=
     * TABLEAU_SIZE. Throws IllegalArgumentException if K is is out of bounds,
     * there is no such card, or the move is illegal
     */
    ...
    private void checkTableauAdd(Card card, Pile p) {
        ...
        if (card.rank() == _base.rank()) {
            throw err("%ss must go to the foundation", card);
