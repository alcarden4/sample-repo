...
class GraphSearch(SearchProblem):
    ...
    def __init__(self, graph_text):
        ...
        if r == None:
            ...
            '"""%s"""' % graph_text
            raise Exception(
                "GraphSearch graph specification start_state not found or incorrect on line:" + l)
        ...
        if r == None:
            ...
            '"""%s"""' % graph_text
            raise Exception(
                "GraphSearch graph specification goal_states not found or incorrect on line:" + l)
...
class PacmanSearchTest(testClasses.TestCase):
    ...
    def getSolInfo(self, search, searchAgents):
        ...
        heuristic = getattr(searchAgents,
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        gold_solution = [str.split(solutionDict['solution']),
                         str.split(solutionDict['rev_solution'])]
        gold_expanded = max(int(solutionDict['expanded_nodes']),
...
class CornerProblemTest(testClasses.TestCase):
    ...
    def solution(self, search, searchAgents):
        ...
        missedCorners = [p for p in ((1, 1), (1, top), (right, 1), (right, top))
                         if p not in visited]
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if type(solution) != type([]):
            grades.addMessage('FAIL: %s' % self.path)
            grades.addMessage(
                'The result must be a list. (Instead, it is %s)' % type(
                    solution))
            return False
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        print
        "Solving problem", self.layoutName
        print
        self.layoutText
...
class HeuristicTest(testClasses.TestCase):
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        print
        "Solving problem", self.layoutName, self.heuristicName
        print
        self.layoutText
...
class HeuristicGrade(testClasses.TestCase):
    def __init__(self, question, testDict):
        ...
        self.thresholds = [int(t) for t in
                           testDict['gradingThresholds'].split()]
